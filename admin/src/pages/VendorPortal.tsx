...
export const VendorPortal: React.FC = () => {
  ...
  const handleSaveEditedProduct = async (e: React.FormEvent) => {
    ...
    try {
      ...
      showToast(`Product "${updatedData.name}" updated successfully! Γ£Å∩╕Å`, 'success');
  ...
  const handleConfirmDeleteProduct = async () => {
    ...
    try {
      ...
      showToast(`Product "${prodName}" deleted successfully! ≡ƒùæ∩╕Å`, 'success', `Reason: ${deleteReason}`);
    } catch (err) {
  ...
  const handleCreateProduct = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (res.ok) {
        showToast('Product submitted for approval! ΓÅ│', 'success', 'It is now in the Admin Approval Queue.');
        // Reset form
        setFormData({
          name: '',
          category: 'STEM & Robotics',
    ...
    <div className="space-y-6">
        ...
        <div className="space-y-4">
          ...
          {loading ? (
            ...
            <div className="bg-white rounded-2xl p-12 text-center border border-slate-200">
              <div className="text-4xl mb-2">≡ƒôª</div>
                      ...
                      <div className="p-4 space-y-2">
                        ...
                        {(!p.stock || p.stock === 0) && (
                          <div className="p-2.5 bg-rose-50 border border-rose-200 rounded-xl text-xs font-bold text-rose-700 flex items-center justify-between gap-2">
                            <span>ΓÜá∩╕Å Stock is empty (0 units left).</span>
                            <button
                              type="button"
                              onClick={() => {
      ...
      {activeTab === 'add' && (
        <div className="bg-white rounded-2xl border border-slate-200 p-6 sm:p-8 max-w-3xl space-y-6">
          ...
          <form onSubmit={handleCreateProduct} className="space-y-4">
            ...
            >
              {submitting ? 'Submitting to Admin Queue...' : 'Submit Toy for Admin Approval ≡ƒÜÇ'}
            </button>
          </form>
        </div>
      )}
            ...
            <div className="p-3.5 bg-rose-50 border border-rose-200 rounded-2xl space-y-1 text-xs">
              ...
              {(!deletingProduct.stock || deletingProduct.stock === 0) && (
                <div className="mt-1 font-bold text-rose-600 flex items-center gap-1">
                  ΓÜá∩╕Å Product has 0 stock (Empty Stock). Deleting will permanently remove it from your inventory.
                </div>
              )}
            </div>
      ...
      {editingProduct && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-3 sm:p-4 bg-slate-900/60 backdrop-blur-xs overflow-y-auto animate-in fade-in duration-200">
          <div className="bg-white rounded-3xl border border-slate-200 shadow-2xl p-4 sm:p-6 lg:p-8 max-w-2xl w-full max-h-[calc(100dvh-100px)] sm:max-h-[90vh] overflow-y-auto space-y-6 my-auto animate-in zoom-in-95 duration-150 relative">
            ...
            <form onSubmit={handleSaveEditedProduct} className="space-y-4">
              ...
              <div className="flex items-center justify-end gap-3 pt-3 border-t border-slate-100">
                ...
                >
                  {updatingProduct ? 'Saving...' : 'Save Product Changes Γ£Å∩╕Å'}
                </button>
              </div>
            </form>
          </div>
      ...
      {selectedOrderDetails && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-900/60 backdrop-blur-xs overflow-y-auto animate-in fade-in duration-200">
          <div className="bg-white rounded-3xl border border-slate-200 shadow-2xl p-6 sm:p-8 max-w-2xl w-full max-h-[90vh] overflow-y-auto space-y-6 my-auto animate-in zoom-in-95 duration-150 relative">
            ...
            <div className="pt-3 border-t border-slate-100 flex flex-col sm:flex-row items-stretch sm:items-center justify-between gap-3 bg-slate-50/90 p-3.5 sm:p-4 rounded-2xl">
              ...
              <button
                type="button"
                onClick={() => setSelectedOrderDetails(null)}
                className="w-full sm:w-auto px-5 py-2.5 bg-slate-800 hover:bg-slate-900 text-white font-bold text-xs rounded-xl cursor-pointer text-center"
              >
                Close Details
              </button>
            </div>
