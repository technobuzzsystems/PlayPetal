...
export const Tabs: React.FC<TabsProps> = ({
  ...
  if (variant === 'pills') {
    return (
      <div className={`flex items-center gap-1.5 p-1 bg-slate-100 border border-slate-200 rounded-xl max-w-fit ${className}`}>
        {tabs.map((tab) => {
        ...
        {tabs.map((tab) => {
          ...
          return (
            <button
              ...
              className={`flex items-center gap-2 py-3 px-1 border-b-2 text-xs font-bold transition-colors cursor-pointer whitespace-nowrap ${
                isActive
